        ...
        for line in inputfile:
            if line.startswith('@@||'):
                ...
                if exceptionrule.startswith('/'):
                    ...
                    print("\tWARNING: convertlists.py does not currently support regex exceptions, and so will ignore them (rule: '{}')".format(
                        exceptionrule))
                ...
                wildcard_domain = wildcard_domain.split(":", 1)[0].split("?", 1)[
                    0].split("/", 1)[0]
...
def convertlist_adbp(inputfile, adbpconfiguration, outputfile):
        ...
        if line.startswith('!'):
            ...
            if len(split_header_line) == 2:
                ...
                if header_key in header_parameters:
                    if verbosity >= 2:
                        print("\tINFO: Duplicate header parameter for key '{}' found. Ignoring.".format(
                            header_key))
                    continue
            ...
            splitline = line[2:].rstrip('\n').split(
                '$', 1)  # Split the rule at any options
    ...
    processinginfo.append(("Non-Domain-only Rules Excluded", lines_processed -
                          comment_lines - domains_retained - domains_excluded - domains_unsupportedoptions))
    processinginfo.append(
        ("Domain-only Rules Excluded (unsupported options)", domains_unsupportedoptions))
    processinginfo.append(
        ("Domain-only Rules Excluded (exception conflict)", domains_excluded))
...
def convertlist_hosts(inputfile, hostsconfiguration, outputfile):
    ...
    for line in inputfile:
        ...
        if line.startswith('#'):
            ...
            if len(split_header_line) == 2:
                header_key = split_header_line[0].lower()
                if header_key in header_parameters:
                    print("\tINFO: Duplicate header parameter for key '{!s}' found. Ignoring.".format(
                        header_key))
                    continue
                header_parameters[header_key] = split_header_line[1]
        elif len(line) == 0:
            ...
            if not ip.is_loopback and not line_components[0] == '0.0.0.0':
                if verbosity >= 3:
                    print(
                        "\tINFO: Ignoring line lacking valid 'blocking' IP address: '{!s}'".format(line))
                nonloopback_lines += 1
            ...
            for host in line_components[1:]:
                ...
                if is_hostfile_ignored_host(host):
                    if verbosity >= 3:
                        print(
                            "\tINFO: Ignoring 'local' host '{!s}' from line: '{!s}'".format(host, line))
                ...
                if not is_valid_hostname(host):
                    if verbosity >= 3:
                        print(
                            "\tINFO: Ignoring invalid host '{!s}' from line: '{!s}'".format(host, line))
                    invalid_hosts += 1
    ...
    headerinfo = dict()
    headerinfo = _hosts_parse_header_comments_for_list_details(
        header_parameters)
    processinginfo = list()
    processinginfo.append(("Total Lines Processed", lines_processed))
    processinginfo.append(("Comment Lines", comment_lines))
...
parser.add_argument(
    "listsjson", help="path to the input `lists.json` file, describing the lists the script should process")
...
# Configuration
converted_output_lists_locations = os.path.join(
    args.outputpath, "lists/")  # type: str
...
with open(args.listsjson) as json_data:
    ...
    if len(json_object) == 0:
        raise ValueError(
            "Root element of lists.json does not contain any entries.")
    ...
    for filterlist in json_object:
        ...
        if 'format' not in filterlist:
            raise ValueError(
                "Missing required 'format' key for list; (string) possible values: \"adbp\", \"hosts\"")
        ...
        if basefilename in unique_output_files:
            ...
            raise ValueError(
                "More than one list in lists.json is currently configured to use the same output filename '{!s}'. To fix this, modify the list dictionaries to specify distinct 'outputfile' filenames".format(basefilename))
        ...
        if parsedurl.scheme == "http" or parsedurl.scheme == "https":
            ...
            if args.verbosity >= 1:
                print("[{!s}]: Fetching list from: {!s} ...".format(
                    filterlist['name'], filterlist['url']))
            locallistfile = os.path.join(args.dlpath, basefilename)
            start_time = time.monotonic()
            r = requests.get(filterlist['url'],
                             stream=True, headers=request_headers)
            if r.status_code != 200:
                print('[{!s}]: ERROR: Unable to download "{!s}"; received HTTP status: {}; skipping'.format(
                    filterlist['name'], filterlist['url'], r.status_code))
                continue
            ...
            if args.verbosity >= 1:
                print("[{!s}]: Fetch completed ({} bytes, in {:.2f} seconds)".format(
                    filterlist['name'], os.path.getsize(locallistfile), (end_time - start_time)))
        elif parsedurl.scheme == "file":
            # Simply use the local file path
            locallistfile = parsedurl.path
            if not os.path.exists(locallistfile):
                print('[{!s}]: ERROR: Input list file does not exist at path: "{!s}"; skipping'.format(
                    filterlist['name'], filterlist['url']))
                continue
        else:
            # Unsupported scheme
            raise ValueError(
                "Unsupported url format: '{!s}'; Expecting 'https://', 'http://', or 'file://' scheme.".format(filterlist['url']))
        ...
        # Convert the list
        if args.verbosity >= 1:
            print("[{!s}]: Processing: {!s} ...".format(
                filterlist['name'], locallistfile))
        start_time = time.monotonic()
        listdetails = dict()
        with open(os.path.join(converted_output_lists_locations, outputfilename), 'w') as f:
            with open(locallistfile, 'r') as inputfile:
                listdetails = convertlist(
                    inputfile, filterlist['format'], formatconfiguration, f)
        end_time = time.monotonic()
        if args.verbosity >= 1:
            print("[{!s}]: Processing completed ({:.2f} seconds)".format(
                filterlist['name'], (end_time - start_time)))
        ...
        list_info['Base Output Filename'] = appendtofilename(
            basefilename, "-{FORMAT}")
        ...
        if args.verbosity >= 1:
            print("[{!s}]: Converted list saved to: {!s}".format(
                filterlist['name'], os.path.join(converted_output_lists_locations, outputfilename)))
...
if args.verbosity >= 1:
    print("Details saved in: {!s}".format(
        os.path.join(args.outputpath, 'details.json')))
